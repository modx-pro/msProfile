        ...
        if (!empty($response)) {
            ...
            foreach ($foundPackages as $foundPackage) {
                if ($foundPackage->name == $packageName) {
                    ...
                    if (version_compare($sig[1] . '-' . $sig[2], $version, '<=')) {
                        continue;
                    }
                    $modx->log(modX::LOG_LEVEL_INFO, "Trying to install <b>{$packageName}</b>. Please wait...");
                    $download = downloadPackage($url,
...
switch (@$options[xPDOTransport::PACKAGE_ACTION]) {
    case xPDOTransport::ACTION_INSTALL:
        ...
        foreach ($packages as $package_name => $version) {
            ...
            $response = installPackage($package_name, $version);
